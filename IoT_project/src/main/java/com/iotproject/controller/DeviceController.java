    ...
    private boolean validateDevice(Device device, boolean isUpdate) {
        if (!isUpdate) {
            ...
            return device.getName() != null && !device.getName().isEmpty()
                    ...
                    && (device.getFloor() != null && device.getFloor() >= 0)  // Para criação, floor não pode ser null e deve ser >= 0
            ...
            boolean isFloorValid = device.getFloor() == null || device.getFloor() >= 0; // Piso é opcional para update
